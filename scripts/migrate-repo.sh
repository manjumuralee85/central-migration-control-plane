...
if [[ -n "$TARGET_JAVA_VERSION" ]]; then
  echo "Target Java version: $TARGET_JAVA_VERSION"
fi
...
normalize_maven_compiler_plugin_java() {
  local java_version="$1"
  [[ -f pom.xml ]] || return 0
  [[ -n "$java_version" ]] || return 0

  # Some repos hardcode maven-compiler-plugin source/target values (e.g. 1.8).
  # Normalize those values to the selected target Java version so PRs are consistent.
  TARGET_JAVA_VERSION="$java_version" perl -0777 -i -pe '
    s{(<plugin>\s*<groupId>org\.apache\.maven\.plugins</groupId>\s*<artifactId>maven-compiler-plugin</artifactId>[\s\S]*?<configuration>[\s\S]*?<source>)\s*[^<]+\s*(</source>)}{$1.$ENV{TARGET_JAVA_VERSION}.$2}gse;
    s{(<plugin>\s*<groupId>org\.apache\.maven\.plugins</groupId>\s*<artifactId>maven-compiler-plugin</artifactId>[\s\S]*?<configuration>[\s\S]*?<target>)\s*[^<]+\s*(</target>)}{$1.$ENV{TARGET_JAVA_VERSION}.$2}gse;
    s{(<plugin>\s*<groupId>org\.apache\.maven\.plugins</groupId>\s*<artifactId>maven-compiler-plugin</artifactId>[\s\S]*?<configuration>[\s\S]*?<release>)\s*[^<]+\s*(</release>)}{$1.$ENV{TARGET_JAVA_VERSION}.$2}gse;
  ' pom.xml
}
...
normalize_maven_compiler_plugin_java "$TARGET_JAVA_VERSION"
